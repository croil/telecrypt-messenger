...
public class MultipartTool implements Closeable {
    ...
    public void appendFile(final String fieldName,
                           final File file, final String path) throws IOException {
        appendFile(fieldName, file, URLConnection.guessContentTypeFromName(file.getName()), path);
    }

    public void appendFile(final String fieldName,
                           final File file,
                           final String contentType, final String path) throws IOException {
        final FileInputStream fis = new FileInputStream(file);
        try {
            appendFile(fieldName, fis, contentType, file.length(), path);
        } finally {
            fis.close();
        }
    }
