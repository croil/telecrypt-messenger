...
public class MultipartExampleClient {
    ...
    public MultipartExampleClient(String path) {
        this.path = path;
    }


    public Pair client(String[] args) throws IOException {
        switch (args.length) {
            case 0:
            case 1:
            ...
            default:
                ...
                return send(url, fields);
        }
        return null;
    }

    private Pair send(final URL url, final List<Field> fields) throws IOException {
            ...
            for (final Field f : fields) {
                switch (f.type) {
                    case TEXT:
                        ...
                        break;
                    case FILE:
                        tool.appendFile(f.name, new File(f.data), this.path);
                        break;
                    case JSON:
        ...
        out.println(connection.getResponseMessage() + " " + code);
        final BufferedReader resp = new BufferedReader(new InputStreamReader(connection.getInputStream()));
        try {
            //noinspection StatementWithEmptyBody
            for (String line; (line = resp.readLine()) != null; id = Integer.parseInt(line));
        } finally {
            resp.close();
        }
        connection.disconnect();
        return new Pair(code, id);
    }
